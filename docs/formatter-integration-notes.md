...
本文件只记录 Skill 需要的既有能力与行为，不修改或复制任何 MCP 实现。
运行时随 Skill 分发的副本位于 [`skills/feishu-doc-formatter/references/formatter-integration-notes.md`](../skills/feishu-doc-formatter/references/formatter-integration-notes.md)；修改集成契约时需同步两处内容。
